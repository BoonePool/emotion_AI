...
import { Download, TrendingUp, PieChart, Activity } from 'lucide-react';
import {
  LineChart,
  Line,
  AreaChart,
...
export default function Report({ session }: ReportProps) {
  // ✅ Keep pulling from the main session JSON, but normalize timeseries so optional fields (like pitch) work safely.
  const rawTimeseries = session?.timeseries ?? [];
  const sessionMetrics = session?.sessionMetrics;

  // Try a few common locations/names for pitch so it still renders if your JSON schema varies a bit.
  ...
  timeseries.forEach((item: any) => {
    Object.keys(emotionSums).forEach((key) => {
      emotionSums[key] += Number(item?.[key] ?? 0);
    });
  });
  ...
  const downloadCSV = () => {
    // ✅ include pitch in CSV as well (same “aspect” as the older code)
    const headers = [
      't_s',
      'engagement',
      'distraction',
    ...
    const rows = timeseries.map((item: any) => [
      item?.t_s ?? '',
      item?.engagement_mean ?? '',
      item?.distracted_mean ?? '',
      item?.pitch ?? '',
      item?.happy ?? '',
      item?.neutral ?? '',
      item?.sad ?? '',
    ...
    const csvContent =
      ...
      [headers.join(','), ...rows.map((r) => r.map((v) => String(v)).join(','))].join('\n');
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm">
            Detailed breakdown of audience engagement and emotional response.
          </p>
        </div>
                ...
                >
                  {pieData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        EMOTION_COLORS[entry.name as keyof typeof EMOTION_COLORS] || '#6366f1'
                      }
                    />
                  ))}
                </Pie>
          ...
          <div className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={timeseries}>
                ...
                {/* ✅ Restored from your previous code: pitch line coming from the main JSON (now normalized) */}
                <Line
                  type="monotone"
                  dataKey="pitch"
                  name="Pitch"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-6">
            <p className="text-zinc-600 text-sm leading-relaxed">
              ...
              High scores indicate a session where the audience was consistently attentive and emotionally
              responsive.
            </p>
          ...
          <div className="flex flex-col items-center justify-center bg-zinc-50 rounded-2xl p-8">
            <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.2em] mb-2">
              Final Score
            </div>
